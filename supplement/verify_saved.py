"""Check the supplied spectral/geometry certificates without an eigensolve."""
...
def verify():
    ...
    rows=json.load(open(data/'exact_audits.json'))
    for row in rows:
        B,hx,hy=row['B'],row['hx'],row['hy']
        candidate=data/f'audit_B{B}_hx{hx}_hy{hy}.npz'
        checked=audit(B=B,D=row['D'],Jz=row['Jz'],hx=hx,hy=hy,load_candidates=candidate)
